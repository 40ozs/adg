...
The important test in this file is :class:`TestEveryRouteIsBehindTheBoundary`. It reads the
OpenAPI document — every route the application actually serves — and calls each one without
a credential. A route added in a later phase and left unprotected fails here, whether or not
anyone remembered to write a test for it. Enumerating endpoints by hand would only ever
prove things about the endpoints somebody thought of.

The database is a stub that raises if a session is opened. That turns a second property
into a test: a request that is going to be rejected must be rejected *before* it costs a
connection, which is the difference between an unauthenticated request being free and being
a way to exhaust the pool.
"""
