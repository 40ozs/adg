...
AD_GRAPH_DIR = pathlib.Path(__file__).parent / "ad_graph"
"""Adversarial identity/membership transcripts (Phase 1C).

These carry only `principal` and `membership_edge` observations, so unlike the canonical
scenarios they replay through the AD ingestion endpoints verbatim. They are generated by
`tests/fixtures/build_ad_graph.py`; a test rebuilds them in memory and compares, so the
committed JSON and the generator cannot drift.
"""
...
def ad_graph_paths() -> list[pathlib.Path]:
    return sorted(AD_GRAPH_DIR.glob("*.json"))


def ad_graph_names() -> list[str]:
    return [path.stem for path in ad_graph_paths()]


def load_ad_graph_raw(name: str) -> dict[str, Any]:
    """Return an adversarial transcript's untouched JSON, for schema validation."""
    path = AD_GRAPH_DIR / f"{name}.json"
    if not path.exists():
        available = ", ".join(ad_graph_names())
        raise FileNotFoundError(f"No AD-graph fixture named {name!r}. Available: {available}")
    document: dict[str, Any] = json.loads(path.read_text(encoding="utf-8"))
    return document


@cache
def load_ad_graph(name: str) -> Scenario:
    """Parse an adversarial transcript through the contract models."""
...
def load_all_ad_graph() -> list[Scenario]:
    return [load_ad_graph(name) for name in ad_graph_names()]
